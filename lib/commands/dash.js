...
CMD.prototype.run = Promise.method(function() {
  ...
  return _this._JAWS.validateProject()
          ...
          return Promise.try(function() {
              ...
              .each(function(resource) {
                ...
                if (resource.type === 'lambda') {
                  if (toggled) _this._report.selectedLambdas++;
                  return CMDtag.tag('lambda', resource.value, !toggled);
                } else if (resource.type === 'endpoint') {
                  if (toggled) _this._report.selectedEndpoints++;
      ...
      .then(function() {
        ...
        if (!_this._allTagged && !_this._report.selectedEndpoints) {
...
CMD.prototype._addAlerts = Promise.method(function() {
  // Status
  JawsCli.log(chalk.white('-------------------------------------------'));
  JawsCli.log(chalk.white(' Dashboard:  Adding Alarms...'));
  JawsCli.log(chalk.white('-------------------------------------------'));
  ...
  var config = {
    ...
    'projectName': _this._JAWS._meta.projectJson.name,
    'moduleName': '',
    'notificationAction': '',
    'functionName': '',
    'alerts': {}
  ...
  var notificationActionBase = 'arn:aws:sns:';

  var region = _this._JAWS._meta.projectJson.stages[config.stageName][0]['region'];
  notificationActionBase += region + ':';

  var roleName = _this._JAWS._meta.projectJson.stages[config.stageName][0]['iamRoleArnLambda'];
  var roleParts = roleName.split(':');
  notificationActionBase += roleParts[roleParts.length - 2] + ':';

  for (var i in _this._choices) {
    var selectableItem = _this._choices[i];

    if (
      typeof(selectableItem.key) != 'undefined' && selectableItem.key == '  L) '
      && typeof(selectableItem.toggled) != 'undefined' && selectableItem.toggled == true
    ) {
      var alertJson = require(selectableItem.value);

      if (typeof(alertJson.alerting) == 'object') {
        config.notificationAction = notificationActionBase + alertJson.alerting.notificationTopicStageMapping[config.stageName];

        var addedAlerts = 0;
        var alertsToAdd = {};
        for (var i in alertJson.alerting.alerts) {
          var alert = alertJson.alerting.alerts[i];
          if (alert.enabled) {
            addedAlerts++;
            alertsToAdd[i] = alert;
          }
        ...
        if (addedAlerts > 0) {
          var jsonFileParts = selectableItem.value.split('/aws_modules/');
          jsonFileParts = jsonFileParts[1].split('/awsm.json');
          jsonFileParts = jsonFileParts[0].split('/');
          config.functionName = jsonFileParts[1];
          config.moduleName = jsonFileParts[0];

          config.alerts = alertsToAdd;

          var alm = new Alerts(config);
        }
      }
    }
  }
